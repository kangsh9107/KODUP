...
<form class="mansearch_search" method="post">
	...
	<div class="row-fluid" style="border-top: 1px solid #dee2e6; margin: 25px 0 25px 0; text-align:center;">
		<div class="col-md-12" style="padding: 20px 10px 0 10px;">
			...
			<input type="search" name="findStr" value="${pVo.findStr }" class="inputSearch" placeholder="구인 내에서 검색" autocomplete="off">
			<input type="search" class="inputSearchTemp">
			<!-- MOVE -->
			<c:choose>
				<c:when test="${pVo.nowPage != pVo.totPage }">
					<label style="float: right; padding: 0 0 0 10px;">
						<input type="button" class="btn btnNext" value="&#xf061;" onclick="mansearch_Move(${pVo.nowPage +1 })" style="font-family: FontAwesome; padding: 0; margin-right: -12px;">
					</label>
				</c:when>
				<c:otherwise>
			...
			<c:choose>
				<c:when test="${pVo.nowPage != 1 }">
					<label style="float: right; padding: 0 0 0 10px;">
						<input type="button" class="btn btnPrevious" value="&#xf060;" onclick="mansearch_Move(${pVo.nowPage -1 })" style="font-family: FontAwesome; padding: 0;">
					</label>
				</c:when>
				<c:otherwise>
					<label style="float: right; padding: 0 0 0 10px; cursor: default; pointer-events: none;">
	...
	<ul>
		<li class="list-group-item" style="padding: 0; margin-top: -10px; color: #555; font-size: 0.9rem;">
			<div style="float: right;">
				<span>${pVo.nowPage }/${pVo.totPage }</span>
			</div>
		</li>
	</ul>	
	<div>
	...
	<ul class="list-group list-group-flush" style="margin-bottom: 40px;">
		<c:forEach var="vo" items="${list }">
			<c:if test="${vo.deadlinecount gt -1 }">
				<li class="list-group-item ListGroupItem" style="padding: 10px 0 10px 0;">
					...
					<br/>
					<c:choose>
						<c:when test="${vo.board_delete == 0 }">
							<div class="index" onclick="mansearch_view(${vo.mansearch_sno})">
								<span id="index_doc" class="index_doc">${vo.position} | ${vo.subject }</span>
							</div>
						</c:when>
						<c:when test="${vo.board_delete ==1 }">
							<label><strong>작성자가 삭제한 글입니다.</strong></label>
						</c:when>
						<c:otherwise>
							<label><strong>관리자가 삭제한 글입니다.</strong></label>
						</c:otherwise>
					</c:choose>
					<c:if test="${vo.board_delete == 0 }">
						<div>
