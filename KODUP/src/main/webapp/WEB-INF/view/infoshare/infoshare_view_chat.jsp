...
<form id="infoshare_view">
	...
	sno<input type='text' id="view_sno" name='sno' size="2" value='${ibVo.sno}' />      <!-- style="visibility: hidden;" 또는 style="display: none;" -->
	repl_sno<input type='text' name='repl_sno' size="2" value='0'/>
	
	board_delete<input type='text'  name='board_delete' size="2" value='${ibVo.board_delete}'/>
	...
	snoID<input type="text" name="sno_id" size="3" value='${ibVo.id}'/>
	...
	<c:choose>
		<c:when test="${ibVo.board_delete eq 0}">
			<!-- 본문 헤더(작성자사진,닉네임,작성일,조회수,보상픽셀) -->
			<div id="infoshare_view_header"  >
				...
				<div  id="infoshare_view_header_section2" style="display:inline-block; vertical-align: top;">
					<div id="infoshare_view_profile_nickname">${ibVo.nickname}</div>
					<span>${ibVo.nal}
			...
			<!-- 본문 글내용 -->
			<span id="infoshare_view_doc" style="font-size:15px; display:inline-block; min-height:300px; margin-top:20px;">
				${ibVo.doc}
			</span>
			<!-- 추천,비추천 -->
				...
				<div>
					<div style="text-align:right;">
						<button id="btn_viewpage_thumbup" type="button" style="display:inline-block; text-align:right; background-color:#fff; border:0;" ><img src="../images/추천엄지.png"></button>
							<input type="text" id="thumb_standard" value="${ibVo.thumbup-ibVo.thumbdown}" style="display:none;"/>
							<span id="thumb" style="font-weight:600px;" >${ibVo.thumbup-ibVo.thumbdown}</span>
			...
			<div id="infoshare_view_hashtag">
				<c:forEach var='vo' items='${ibVo.hashtaglist}'>
					<span>
						<a href="#" onclick="infoshare_view_findHashtag(${vo.hashtag})">${vo.hashtag}</a>
					</span>
				</c:forEach>
			</div>
			<!-- 본문 btnzone -->
			...
			<!-- 댓글insert-->
			<c:if test="${sessionScope.sessionId ne null }">
				<div id="infoshare_view_repl_insert" style=" border:1px solid #DCDCDC; padding : 20px 30px 50px 30px; border-radius:30px;">
					<!-- 댓글인서트 서머노트 -->
					<div class="view_summernote_section">
						<span>
							<img id="infoshare_view_repl_insert_profileimage_file" src="../upload/${sessionScope.profile_img}" style="border-radius:50%;">
						</span>
						<textarea id="view_main_summernote" class="view_summernote"></textarea>
					</div>
					<div>
						<input type="button" value="댓글입력"
						id="infoshare_view_repl_btnInsert" 
						class="infoshare_view_originalBtn" 
			...
			<c:if test="${sessionScope.sessionId eq null}">
				<div id="infoshare_view_repl_insert" style=" border:1px solid #DCDCDC; padding : 20px 30px 50px 30px; border-radius:30px;">
					...
					<div class="view_summernote_section">
						<span>
							<img id="infoshare_view_repl_insert_profileimage_file" src="../images/NULLboy.png" style="border-radius:50%;">
						</span>
			...
			<hr/>
			<!-- 댓글리스트 -->
			<div id="infoshare_view_repl_list" style="margin-top:50px;"> 
				...
				<div style="display:none;">
					...
					<c:if test ='${replList.deep eq 0}'>
						...
						<div class="infoshare_view_repl_profile" style="font-size:13px; margin-top:15px;">
							<span class="infoshare_view_repl_profile_profileimage"><!--댓글작성자프로필사진+닉네임+댓글작성시간 한줄로 붙이기위해 inline요소인 span태그사용 -->
								<img class="infoshare_view_repl_profile_profileimage_file" src="../upload/${replList.profile_img}"style="border-radius:50%;" >
							</span>
							
							<span class="infoshare_view_repl_profile_nickname" 
						...
						<div class="infoshare_view_repl_btnzone">
							<c:if test="${replList.repl_delete eq 0}">
								...
								<c:if test="${replList.id eq sessionScope.sessionId}">
									<input type="button" class="infoshare_view_originalBtn" value="수정"
									onclick="view_update_Repl_open(${replList.repl_sno})">
									<input type="button" class="infoshare_view_originalBtn" value="삭제"
						...
						<div id="updateReplSection${replList.repl_sno}"style="display:none;">
							...
							<button type="button" class="infoshare_view_originalBtn" onclick="updateReplUpdate(${replList.repl_sno})">수정입력</button>
							<button type="button" class="infoshare_view_originalBtn" onclick="updateReplCancel(${replList.repl_sno})">취소</button>
						</div>
					
						<!-- (ON/OFF)display ; 대댓글입력폼 -->
						<div id="repl_insert_section${replList.repl_sno}" style="display:none; font-size:15px; margin-left:50px;">
							<span>
								<img id="infoshare_view_repl_inner_insert_profileimage_file" src="../upload/${sessionScope.profile_img}" style="border-radius:50%;"><!-- 세션아이디의 프로필이미지가 들어감 -->
							</span>
							<!-- 대댓글인서트_내용 서머노트 -->
							<div id="view_inner_summernote${replList.grp}"class="view_summernote"></div>
							<!-- 대댓글인서트_버튼 -->
							<div id="infoshare_view_repl_inner_btnInsert" >
								<input type="button" class="infoshare_view_originalBtn" value="대댓글입력"
								style="width:100px;"
					...
					<c:if test ='${replList.deep ne 0}'>
						<div class="repl_inner_section${replList.grp}"style="margin-left:50px; ">
							...
							<!-- 댓글내용 -->
							<div class="infoshare_view_repl_doc" style="position:relative; font-size:13px; border:1px solid #E6E6E6; border-radius:10px; padding:2px;">
								<div>
									<span>
										<c:choose>
											<c:when test="${replList.repl_delete eq 0}">
		...
		<c:when test="${ibVo.board_delete eq 1}">
			<!-- 본문 헤더(작성자사진,닉네임,작성일,조회수,보상픽셀) -->
			<div id="infoshare_view_header"  >
				<span id="infoshare_view_header_section1"> <!-- span태그사용 : 이미지사진+닉네임 한줄로 붙이기위해 span이 inline요소라서 -->
					<img id="infoshare_view_header_profileimage_file" 
		...
		<c:when test="${ibVo.board_delete eq 2}">
			<!-- 본문 헤더(작성자사진,닉네임,작성일,조회수,보상픽셀) -->
			<div id="infoshare_view_header"  >
				<span id="infoshare_view_header_section1"> <!-- span태그사용 : 이미지사진+닉네임 한줄로 붙이기위해 span이 inline요소라서 -->
					<img id="infoshare_view_header_profileimage_file" 
