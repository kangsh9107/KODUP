...
<body>
	...
	<div class="container" style="padding: 0;">
		...
		<form name="board_search_form" class="board_search_form" method="post">
			<div class="row">
				<div class="col-md-12" style="padding-left: 10px; padding-right: 10px;">
					...
					<!-- HORSEHEAD -->
					<input type="radio" onclick="runQna('스포츠')" class="btn-check" name="horsehead_radio" id="horsehead_radio1" ${cbpVo.horsehead eq '스포츠' ? 'checked' : '' }>
					<label for="horsehead_radio1">스포츠</label>
					<input type="radio" onclick="runQna('게임')" class="btn-check" name="horsehead_radio" id="horsehead_radio2" ${cbpVo.horsehead eq '게임' ? 'checked' : '' }>
					<label for="horsehead_radio2">게임</label>
					<input type="radio" onclick="runQna('일상')" class="btn-check" name="horsehead_radio" id="horsehead_radio3" ${cbpVo.horsehead eq '일상' ? 'checked' : '' }>
					<label for="horsehead_radio3">일상</label>
					<input type="radio" onclick="runQna('연예')" class="btn-check" name="horsehead_radio" id="horsehead_radio4" ${cbpVo.horsehead eq '연예' ? 'checked' : '' }>
					<label for="horsehead_radio4">연예</label>
					<input type="radio" onclick="runQna('기타')" class="btn-check" name="horsehead_radio" id="horsehead_radio5" ${cbpVo.horsehead eq '기타' ? 'checked' : '' }>
					<label for="horsehead_radio5">기타</label>
					<input type="radio" onclick="runQna('')" class="btn-check" name="horsehead_radio" id="horsehead_radio6" ${cbpVo.horsehead eq '' ? 'checked' : '' }>
			...
			<div class="row-fluid" style="border-top: 1px solid #dee2e6; margin: 25px 0 25px 0;">
				<div class="col-md-12" style="padding: 20px 10px 0 10px;">
					...
					<input type="search" name="findStr" value="${cbpVo.findStr }" class="inputSearch" placeholder="커뮤니티 내에서 검색" autocomplete="off">
		...
		<ul class="list-group list-group-flush" style="margin-bottom: 40px;">
			...
			<c:forEach items="${listFreetalking }" var="listFreetalking">
				<li class="list-group-item" style="padding: 10px 0 10px 0;">
					<a href="javascript:void(0);" onclick="runTopWriterProfile('${listFreetalking.nickname }')" class="text-decoration-none">
						<img id="index_profile" src="upload/${listFreetalking.profile_img }">
						<span id="index_nickname">${listFreetalking.nickname }</span>
					</a>
					<span id="index_time">· ${listFreetalking.nal }</span>
					<div style="float: right;">
						<!-- VIEW CHECK -->
						<c:choose>
							<c:when test="${listFreetalking.viewStatus == 0 }">
					...
					<a href="#" onclick="freetalking_view(${listFreetalking.sno })" class="text-decoration-none">
						<span id="index_subject">
							<c:choose>
								<c:when test="${listFreetalking.board_delete == 0 }">
									${listFreetalking.subject }
								</c:when>
								<c:when test="${listFreetalking.board_delete == 1 }">
									작성자가 삭제한 글입니다.
								</c:when>
								<c:otherwise>
									관리자가 삭제한 글입니다.
					...
					<div style="float: left;">
						<!-- HORSEHEAD -->
						<label id="board_horsehead">${listFreetalking.horsehead }</label>
						<!-- HASHTAG -->
						<c:forTokens items="${listFreetalking.hashtag }" var="hashtag" delims="#">
					...
					<div style="float: right; cursor: default;">
						...
						<span id="index_viewcount_sum">${listFreetalking.viewcount }</span>
						<img id="index_updown" src="images/index_updown.png">
						<span id="index_updown_sum">${listFreetalking.thumb }</span>
						<img id="index_repl" src="images/index_repl.png">
						<span id="index_repl_sum">${listFreetalking.countRepl }</span>
					</div>
				</li>
			</c:forEach>
		</ul>
