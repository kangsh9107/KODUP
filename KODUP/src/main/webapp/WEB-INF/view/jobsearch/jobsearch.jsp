		...
		<form name="board_search_form" class="board_search_form" method="post">
			<div class="row">
				<div class="col-md-12" style="padding-left: 10px; padding-right: 10px;">
					...
					<c:choose>
						...
						<c:otherwise>
							<input type="button" style="visibility: hidden;" class="btn btnJobsearchInsert" value="&#xf304; 작성하기" style="font-family: FontAwesome; float: left;">
						</c:otherwise>
					</c:choose>
					<!-- HORSEHEAD -->
					<input type="radio" onclick="runQna('백엔드')" class="btn-check" name="horsehead_radio" id="horsehead_radio1" ${cbpVo.horsehead eq '백엔드' ? 'checked' : '' }>
					<label for="horsehead_radio1">백엔드</label>
					<input type="radio" onclick="runQna('프론트엔드')" class="btn-check" name="horsehead_radio" id="horsehead_radio2" ${cbpVo.horsehead eq '프론트엔드' ? 'checked' : '' }>
					<label for="horsehead_radio2">프론트엔드</label>
					<input type="radio" onclick="runQna('풀스택')" class="btn-check" name="horsehead_radio" id="horsehead_radio3" ${cbpVo.horsehead eq '풀스택' ? 'checked' : '' }>
					<label for="horsehead_radio3">풀스택</label>
					<input type="radio" onclick="runQna('기타')" class="btn-check" name="horsehead_radio" id="horsehead_radio4" ${cbpVo.horsehead eq '기타' ? 'checked' : '' }>
					<label for="horsehead_radio4">기타</label>
					<input type="radio" onclick="runQna('')" class="btn-check" name="horsehead_radio" id="horsehead_radio5" ${cbpVo.horsehead eq '' ? 'checked' : '' }>
			...
			<div class="row-fluid" style="border-top: 1px solid #dee2e6; margin: 25px 0 25px 0;">
				<div class="col-md-12" style="padding: 20px 10px 0 10px;">
					...
					<input type="search" name="findStr" value="${cbpVo.findStr }" class="inputSearch" placeholder="구직 내에서 검색" autocomplete="off">
...
>>>>>>> refs/remotes/origin/seongho
		...
		<ul class="list-group list-group-flush" style="margin-bottom: 40px;">
			...
			<c:forEach items="${listJobsearch }" var="listJobsearch">
				<li class="list-group-item" style="padding: 10px 0 10px 0;">
					<a href="javascript:void(0);" onclick="runTopWriterProfile('${listJobsearch.nickname }')" class="text-decoration-none">
						<img id="index_profile" src="upload/${listJobsearch.profile_img }">
						<span id="index_nickname">${listJobsearch.nickname }</span>
					</a>
					<span id="index_time">· ${listJobsearch.nal }</span>
					<div style="float: right;">
						<!-- VIEW CHECK -->
						<c:choose>
							<c:when test="${listJobsearch.viewStatus == 0 }">
...
<<<<<<< HEAD
					<a href="#" onclick="jobsearch_view(${listJobsearch.sno })" class="text-decoration-none">
						<span id="index_subject">
							<c:choose>
								<c:when test="${listJobsearch.board_delete == 0 }">
									${listJobsearch.subject }
								</c:when>
								<c:when test="${listJobsearch.board_delete == 1 }">
									작성자가 삭제한 글입니다.
								</c:when>
								<c:otherwise>
									관리자가 삭제한 글입니다.
...
>>>>>>> refs/remotes/origin/seongho
					...
					<div style="float: left;">
						<!-- HORSEHEAD -->
						<label id="board_horsehead">${listJobsearch.horsehead }</label>
						<!-- HASHTAG -->
						<c:forTokens items="${listJobsearch.hashtag }" var="hashtag" delims="#">
					...
					<div style="float: right; cursor: default;">
						...
						<span id="index_viewcount_sum">${listJobsearch.viewcount }</span>
						<img id="index_updown" src="images/index_updown.png">
						<span id="index_updown_sum">${listJobsearch.thumb }</span>
						<img id="index_repl" src="images/index_repl.png">
						<span id="index_repl_sum">${listJobsearch.countRepl }</span>
					</div>
				</li>
			</c:forEach>
		</ul>
