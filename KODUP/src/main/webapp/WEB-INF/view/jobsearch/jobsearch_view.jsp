...
<form id="jobsearch_view">
	...
	<c:choose>
		<c:when test="${jbVo.board_delete eq 0}">
			...
			<div id="jobsearch_view_hashtag">
				<c:forEach var='vo' items='${jbVo.hashtaglist}'>
					<span>
						<a href="#" onclick="jobsearch_view_findHashtag(${vo.hashtag})">${vo.hashtag}</a>
					</span>
				</c:forEach>
			</div>
			<!-- 본문 btnzone -->
			<div id="jobsearch_view_btnzone" style="margin-top:30px;" >
				<c:if test="${jbVo.id eq sessionScope.sessionId}">
					<input type="button" class="btnUpdate jobsearch_view_originalBtn" style="vertical-align:top;"value="수정">
					<input type="button" class="btnDeleteR  jobsearch_view_originalBtn"style="vertical-align:top;" value="삭제">
				</c:if>
			...
			<c:if test="${sessionScope.sessionId ne null }">
				<div id="jobsearch_view_repl_insert" style=" border:1px solid #DCDCDC; padding : 20px 30px 50px 30px; border-radius:30px;">
					...
					<div class="view_summernote_section">
						<span>
							<img id="jobsearch_view_repl_insert_profileimage_file" src="upload/${sessionScope.profile_img}" style="border-radius:50%;">
						</span>
					...
					<c:if test ='${replList.deep eq 0}'>
						...
						<div class="jobsearch_view_repl_profile" style="font-size:13px; margin-top:15px;">
							<span class="jobsearch_view_repl_profile_profileimage"><!--댓글작성자프로필사진+닉네임+댓글작성시간 한줄로 붙이기위해 inline요소인 span태그사용 -->
								<img class="jobsearch_view_repl_profile_profileimage_file" src="upload/${replList.profile_img}"style="border-radius:50%;" >
							</span>
						...
						<div id="repl_insert_section${replList.repl_sno}" style="display:none; font-size:15px; margin-left:50px;">
							<span>
								<img id="jobsearch_view_repl_inner_insert_profileimage_file" src="upload/${sessionScope.profile_img}" style="border-radius:50%;"><!-- 세션아이디의 프로필이미지가 들어감 -->
							</span>
							<!-- 대댓글인서트_내용 서머노트 -->
