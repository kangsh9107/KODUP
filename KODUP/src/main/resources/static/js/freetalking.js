...
function freetalking_view_findHashtag(hashtag){
...
$('#btn_viewpage_thumbup').on('click', function(){
    var param = $('#freetalking_view').serialize();
    var thumb = $('#thumb').text();
    $('#thumb').text(Number(thumb)+1);
    $.post("/freetalking/freetalking_view/thumbup", param, function(data){})
...
$('#btn_viewpage_thumbdown').on('click', function(){
    ...
    var thumb = $('#thumb').text();
    $('#thumb').text(Number(thumb)-1);
    $.post("/freetalking/freetalking_view/thumbdown", param, function(data){})
    	var thumb_standard = $('#thumb_standard').val();
    if(Number(thumb) <= thumb_standard){
...
$('.btnDeleteR').on('click', function(){
    ...
    var param = $('#freetalking_view').serialize();
    $.post("/freetalking/freetalking_view/deleteR", param, function(data){
       $('#center').html(data);
    })
})
...
function view_repl_deleteR(repl_sno){
    ...
    var param = $('#freetalking_view').serialize();
    $.post("/freetalking/freetalking_view/ReplDeleteR", param, function(data){
       $('#center').html(data);
    })
}
...
function view_insert_repl(){
	...
	var param = $('#freetalking_view').serialize();
    $.post("/freetalking/freetalking_view/insertRepl", param, function(data){
       $('#center').html(data);
    })
...
//대댓서머노트 코드 가져오기
	...
	var param = $('#freetalking_view').serialize();
    $.post("/freetalking/freetalking_view/insertInnerRepl", param, function(data){
       $('#center').html(data);
    })
...
function updateReplUpdate(repl_sno){
    ...
    var param = $('#freetalking_view').serialize();
    $.post("/freetalking/freetalking_view/ReplUpdateR", param, function(data){
       $('#center').html(data);
     })
}